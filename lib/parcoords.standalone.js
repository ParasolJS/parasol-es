...
(function (global, factory) {
    ...
    var pathUtopia = function pathUtopia(config, ctx, position) {
      return function (d, i) {
        ctx.utopia.strokeStyle = _functor(config.color)(d, i);
        return colorPath(config, position, d, ctx.utopia);
      };
    };

    // draw the utopia line
    var utopiaLine = function utopiaLine(config, pc, canvas, ctx, position) {
      return function () {
        // flags.utopia = true;

        // find the min value of each column and create a (key, value) entry for utopia data
        var utopia_obj = {};
        Object.entries(config.dimensions).forEach(
          ([key, value]) =>  {
    ...
    var ParCoords = function ParCoords(userConfig) {
      ...
      pc.utopiaLine = utopiaLine(config, pc, position);
