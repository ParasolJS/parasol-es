...
(function (global, factory) {
    ...
    var autoscale = function autoscale(config, pc, xscale, ctx) {
      return function () {
        ...
        ctx.utopia.linewidth = 3;
        ctx.utopia.strokeStyle = "1a1a1a";
        ctx.utopia.scale(devicePixelRatio, devicePixelRatio);
    ...
    var reorder = function reorder(config, pc, xscale) {
      return function (rowdata) {
        ...
        if (reordered) {
          ...
          var utopia = config.utopia.slice(0);
          config.utopia = [];
          pc.clear('utopia');
    ...
    var pathUtopia = function pathUtopia(config, ctx, position) {
      return function (d, i) {
        ctx.utopia.lineWidth = 4;
        return colorPath(config, position, d, ctx.utopia);
      };
    };

    var renderUtopiaDefault = function renderUtopiaDefault(config, ctx, position, pc) {
      return function () {
        pc.clear('utopia');

        if (config.utopia.length) {
          config.utopia.forEach(pathUtopia(config, ctx, position));
        }
      };
    };

    var renderUtopiaQueue = function renderUtopiaQueue(config, utopiaQueue) {
      return function () {
        if (config.utopia) {
          utopiaQueue(config.utopia);
        } else {
          utopiaQueue([]); // This is needed to clear the line
        }
      };
    };

    var renderUtopia = function renderUtopia(config, pc, events) {
      return function () {
        if (!Object.keys(config.dimensions).length) pc.detectDimensions();

        //pc.renderMarked[config.mode]();
        events.call('add_utopia', this);
        return this;
      };
    };

    // draw the utopia line
    var utopiaLine = function utopiaLine(config, pc, canvas, events, ctx, position) {
      return function (optimize_list) {
        // find the optimal value of each column and create a (key, value) entry for utopia data

        // NOTE: optimize_list should be an object with axes names as keys and a string indicating whether the 'max' or 'min' direction is optimal
        var utopia_obj = {};
        Object.entries(optimize_list).forEach(
          ([key, value]) =>  {
            if(config.dimensions[key]){
              if(value == "min") {
                utopia_obj[key] = pc.dimensions()[key].yscale.domain()[0];
              } else {
                utopia_obj[key] = pc.dimensions()[key].yscale.domain()[1];
                pc.flip(key);
                pc.render().updateAxes(0);
              }
            }
        });

        config.utopia = [utopia_obj];
        pc.clear('utopia');
        console.log(config.utopia);
        config.utopia.forEach(pathUtopia(config, ctx, position));
        events.call('add_utopia', this, config.utopia);
        return this;
      };
    };
    ...
    var init$1 = function init(config, canvas, ctx) {
      ...
      var pc = function pc(selection$$1) {
        ...
        // canvas data layers
        ['dots', 'foreground', 'brushed', 'highlight', 'marked', 'utopia'].forEach(function (layer) {
    ...
    var initState = function initState(userConfig) {
      ...
      var eventTypes = ['render', 'resize', 'highlight', 'mark', 'add_utopia', 'brush', 'brushend', 'brushstart', 'axesreorder'].concat(keys(config));
    ...
    var bindEvents = function bindEvents(__, ctx, pc, xscale, flags, brushedQueue, markedQueue, utopiaQueue, foregroundQueue, events, axis) {
      var side_effects = sideEffects(__, ctx, pc, xscale, flags, brushedQueue, markedQueue, utopiaQueue, foregroundQueue);
    ...
    var ParCoords = function ParCoords(userConfig) {
      ...
      var utopiaQueue = renderQueue(pathUtopia(config, ctx, position)).rate(50).clear(function () {
        return pc.clear('utopia');
      });
      ...
      bindEvents(config, ctx, pc, xscale, flags, brushedQueue, markedQueue, utopiaQueue, foregroundQueue, events, axis);
      ...
      pc.renderUtopia = renderUtopia(config, pc, events);
      ...
      pc.renderUtopia.default = renderUtopiaDefault(config, ctx, position, pc);
      pc.renderUtopia.queue = renderUtopiaQueue(config, utopiaQueue);
      ...
      pc.utopiaLine = utopiaLine(config, pc, canvas, events, ctx, position);
