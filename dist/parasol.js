(function (global, factory) {
    typeof exports === 'object' && typeof module !== 'undefined' ? module.exports = factory(require('d3'), require('parcoord-es'), require('slickgrid-es6'), require('lodash-es'), require('d3-dsv'), require('ml-kmeans')) :
    typeof define === 'function' && define.amd ? define(['d3', 'parcoord-es', 'slickgrid-es6', 'lodash-es', 'd3-dsv', 'ml-kmeans'], factory) :
    (global.Parasol = factory(global.d3,global.ParCoords,global.SlickGrid,global.lodashEs,global.d3Dsv,global.kmeans));
}(this, (function (d3,ParCoords,SlickGrid,lodashEs,d3Dsv,kmeans) { 'use strict';

    ParCoords = ParCoords && ParCoords.hasOwnProperty('default') ? ParCoords['default'] : ParCoords;
    SlickGrid = SlickGrid && SlickGrid.hasOwnProperty('default') ? SlickGrid['default'] : SlickGrid;
    kmeans = kmeans && kmeans.hasOwnProperty('default') ? kmeans['default'] : kmeans;
    ...
    var attachGrid = function attachGrid(config, ps, flags) {
      return function (_ref) {
        ...
        if (columns === null) {
          ...
          column_keys = lodashEs.difference(column_keys, ['id']);
    ...
    var gridUpdate = function gridUpdate(config, ps, flags) {
      return function () {
        ...
        if (config.marked.length) {
          data = lodashEs.union(data, config.marked);
        }
    ...
    var sync = function sync(config, ps, flags) {
      return function () {
        ...
        if (lodashEs.union.apply(undefined, brush_extents).length == 0) {
          ps.linked.forEach(function (pc) {
            pc.brushReset();
          });
          config.brushed = [];
          ...
          var brushed = lodashEs.intersection.apply(undefined, brush_extents);
          ps.linked.forEach(function (pc) {
    ...
    var format_data = function format_data(data) {
      return d3Dsv.csvParse(d3Dsv.csvFormat(data));
    };
    ...
    var add_column = function add_column(columns, col_name) {
      ...
      if (lodashEs.includes(names, col_name)) {
        return columns;
      } else {
        var col_def = {
          id: col_name,
    ...
    var cluster = function cluster(config, ps, flags) {
      return function () {
        var _ref = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {},
            ...
            palette = _ref$palette === undefined ? d3.schemeCategory10 : _ref$palette,
            _ref$options = _ref.options,
        ...
        if (Array.isArray(palette)) {
          var scheme = d3.scaleOrdinal(palette);
    ...
    var hideAxes = function hideAxes(config, ps, flags) {
      return function (partition) {
        if (Array.isArray(partition)) {
          ...
          Object.keys(config.partition).forEach(function (id) {
            config.partition[id] = lodashEs.union(config.partition[id], partition);
          });
        } else if (lodashEs.isPlainObject(partition)) {
          ...
          Object.entries(partition).forEach(function (_ref) {
            ...
            if (config.partition[key]) {
              config.partition[key] = lodashEs.union(config.partition[key], partition[key]);
            }
          });
        } else {
    ...
    var showAxes = function showAxes(config, ps, flags) {
      return function (partition) {
        if (typeof partition === 'undefined') {
          ...
          Object.keys(config.partition).forEach(function (id) {
            config.partition[id] = lodashEs.difference(config.partition[id], partition);
          });
        } else if (lodashEs.isPlainObject(partition)) {
          ...
          Object.entries(partition).forEach(function (_ref) {
            ...
            if (config.partition[key]) {
              config.partition[key] = lodashEs.difference(config.partition[key], partition[key]);
            }
          });
        } else {
          throw 'Error: please provide an object or array as argument.';
    ...
    var setAxesLayout = function setAxesLayout(config, ps, flags) {
      return function (layout) {
        if (lodashEs.isPlainObject(layout)) {
          ...
          Object.entries(layout).forEach(function (_ref) {
            ...
            if (config.partition[key]) {
              config.partition[key] = lodashEs.difference(config.vars.concat('id'), layout[key]);
            }
          });
        } else {
    ...
    var exportData = function exportData(config, ps, flags) {
      return function (_ref) {
        ...
        if (d.length > 0) {
          ...
          var csv = d3.csvFormat(d, config.vars);
    ...
    var DefaultConfig = {
      ...
      selections: function selections() {
        return lodashEs.union(this.brushed, this.marked);
      }
    };
    ...
    var initState = function initState(data, userConfig) {
      ...
      // 'highlight',
      // 'mark',
      'brush', 'brushend', 'brushstart'].concat(d3.keys(config));
